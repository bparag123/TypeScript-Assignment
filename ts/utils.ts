...
export default class CalculatorUtils {
    ...
    static getFactorialKeyword = () => {
        let getFactorialIndexes:number[] = [];
        ...
        let searchedExpressions:string[] = [];
        for (let i = 0; i < getFactorialIndexes.length; i++) {
            ...
            while (prev >= 0) {
                if (varManager.getData().formula[prev] === ")") parenthesisCount++;
                if (varManager.getData().formula[prev] === "("){
                    if(parenthesisCount === 0) break
                    parenthesisCount--;
                }
                if (operators.includes(varManager.getData().formula[prev]) && parenthesisCount == 0) break;
